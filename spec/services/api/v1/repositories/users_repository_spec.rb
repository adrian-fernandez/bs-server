...
RSpec.describe Api::V1::Repositories::UsersRepository do
  describe '.search_filters' do
    it 'should return array of Filters' do
      expect(subject.send(:search_filters)).to eq(
        [
          Api::V1::Filters::UserFilters::ExcludeRoleIdsFilter,
          Api::V1::Filters::UserFilters::EmailFilter,
  ...
  describe '.service_name' do
    it 'should return the name of the service' do
      expect(subject.send(:service_name).to_s).to eq('User')
    end
  end

  describe '.sort_service' do
    it 'should return the name of the sorting service' do
      expect(subject.send(:sort_service).to_s).to eq('Api::V1::Sorters::UserSorter')
    end
  end

  describe '.filter_service' do
    it 'should return the name of the filtering service' do
      expect(subject.send(:filter_service).to_s).to eq('Api::V1::Filters::UserFilters')
    end
  end

  describe 'select fields' do
    context 'when there are selected fields' do
      let(:subject_with_params) { Api::V1::Repositories::UsersRepository.new(params: { selected_fields: ['id'] }) }
  ...
  describe '.paginate?' do
    ...
    it 'should return false when it has param paginate=false' do
      subject_with_params = Api::V1::Repositories::UsersRepository.new(params: { paginate: 'false' })
  ...
  describe '.no_selectable_fields' do
    it 'should return an empty array' do
      expect(subject.send(:no_selectable_fields)).to eq([:encrypted_password])
    end
  end
end
